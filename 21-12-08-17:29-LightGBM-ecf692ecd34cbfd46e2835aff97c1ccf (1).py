...
dataset = spark.read.table("dec21_flightschool_team2.turbines_gold").drop('status')
display(dataset)

pandas_data = dataset.sample(.000001).drop('status').toPandas()
...
df_selected_columns = preprocessor.fit_transform(pandas_data)
...
df = pd.DataFrame(df_selected_columns)
df.shape

# COMMAND ----------

df_json= df.to_json()
dbutils.fs.put("/home/nicholas.barretta@databricks.com/inference_text_5.txt", df_json)
...
lgbmc_classifier = LGBMClassifier(
  colsample_bytree=0.40582863469669783,
  lambda_l1=1.0967772382793919,
  lambda_l2=4.210458941941947,
  learning_rate=0.5780736656873661,
  max_bin=352,
  max_depth=9,
  min_child_samples=343,
  n_estimators=512,
  num_leaves=4,
  path_smooth=43.41047637180449,
  subsample=0.5038782567832643,
